...
contract('SupplyChain', function (accounts) {
  ...
  it('Testing smart contract function markItemForSale() that allows a farmer to sell coffee', async () => {
    ...
    // prepare test
    await supplyChain.markItemForSale(upc, productPrice, {
      from: originFarmerID,
    })
  ...
  async function assertResultBufferOne(expectedState) {
    ...
    assert.equal(
      resultBufferOne[2],
      getExpectedItemOwner(expectedState),
      'Error: Missing or Invalid ownerID',
    )
    assert.equal(
      resultBufferOne[3],
      originFarmerID,
      'Error: Missing or Invalid originFrmerID',
    )
    assert.equal(
      resultBufferOne[4],
      originFarmName,
  ...
  async function assertResultBufferTwo(expectedState) {
    ...
    assert.equal(
      resultBufferTwo[4],
      expectedState >= State.ForSale ? productPrice : 0,
      'Error: Missing or Invalid productPrice',
    )
    assert.equal(resultBufferTwo[5], expectedState, 'Error: Invalid item State')
    assert.equal(
      resultBufferTwo[6],
      expectedState >= State.Sold ? distributorID : emptyAddress,
      'Error: Missing or Invalid distributorID',
    )
    assert.equal(
      resultBufferTwo[7],
      expectedState >= State.Received ? retailerID : emptyAddress,
      'Error: Missing or Invalid retailerID',
    )
    assert.equal(
      resultBufferTwo[8],
      expectedState >= State.Purchased ? consumerID : emptyAddress,
